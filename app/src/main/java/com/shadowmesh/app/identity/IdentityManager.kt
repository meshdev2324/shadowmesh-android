...
class IdentityManager @Inject constructor(
     ...
     */
    suspend fun registerPasskey(userId: String, context: android.content.Context): Result<Unit> = withContext(ioDispatcher) {
        val client = vpnRepository.apiClient
        try {
     ...
     */
    suspend fun loginWithPasskey(rawCode: String, context: android.content.Context): Result<ActivationResponse> = withContext(ioDispatcher) {
        val client = vpnRepository.apiClient
        try {
            ...
            val responseJson = result.credential.data.getString("androidx.credentials.BUNDLE_KEY_REGISTRATION_RESPONSE_JSON")
