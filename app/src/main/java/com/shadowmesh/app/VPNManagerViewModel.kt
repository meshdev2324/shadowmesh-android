...
class VPNManagerViewModel @Inject constructor(
    ...
    private fun observeManagers() {
        ...
        // Horizon 4: Monitor XR Display Glasses Connection
        viewModelScope.launch {
            // Simulated: In production this uses ProjectedContext or DisplayManager
            while (isActive) {
                val isConnected = false // TODO: Integration with XR SDK
                updateUiState { it.copy(isGlassesConnected = isConnected) }
                delay(5000)
            }
        }
    }
    ...
    private fun loadInitialState() {
        viewModelScope.launch(ioDispatcher) {
            ...
            // Refresh security manifest for phased rollout (SOP 14)
            launch {
                try {
                    val manifest = vpnRepository.apiClient.fetchSecurityManifest()
                    updateUiState { it.copy(featureFlags = manifest.flags) }
                } catch (e: Exception) {
                    android.util.Log.e("VPNViewModel", "Failed to fetch security manifest", e)
                }
            }

            // Refresh nodes from network in background using resilient discovery (RFC-004)
            try {
                val freshNodes = vpnRepository.apiClient.getNodesResilient()
                if (freshNodes.isNotEmpty()) {
                    updateUiState { it.copy(nodes = freshNodes, isDiscoveryResilient = true) }
                }
            } catch (e: Exception) {
