...
class ConnectionManager @Inject constructor(
    ...
    fun toggleConnection(
        ...
        if (vpnRepository.getStatus() == ConnectionStatus.CONNECTED) {
            ...
            val keys = wireguardService.generateKeyPair()
            connect(node, keys.second, preference, isCamouflage, dnsLeakProtection)
        }
    }
    ...
    private fun connect(
        ...
        connectionJob = scope.launch {
            connectUseCase.execute(node, publicKey, preference, isCamouflage, dnsLeakProtection)
                .collect { progress ->
