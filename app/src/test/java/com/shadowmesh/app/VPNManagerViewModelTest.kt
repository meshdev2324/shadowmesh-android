...
class VPNManagerViewModelTest {
    ...
    fun `initialization updates state from repository with resilient nodes`() =
        runTest {
            val testNodes =
                listOf(
                    VpnNode("id1", "Node 1", "US", "USA", "1.2.3.4", "pub", 10u, 50u, true),
                )
            coEvery { vpnRepository.apiClient.getNodesResilient() } returns testNodes
            every { vpnRepository.getStatus() } returns ConnectionStatus.DISCONNECTED
    ...
    fun `connection manager progress updates UI state`() =
        runTest {
            createViewModel()
            val connectingProgress = ConnectionProgress.Connecting(ConnectionStatus.CONNECTING_DIRECT, "Securing tunnel...", null)
    ...
    fun `toggleConnection delegates to connectionManager`() =
        runTest {
            val testNode = VpnNode("id1", "Node 1", "US", "USA", "1.2.3.4", "pub", 10u, 50u, true)
            // Mock state
    ...
    fun `selectNode updates current node`() = runTest {
        val node = VpnNode("id", "N", "US", "USA", "1.1.1.1", "pub", 0u, 0u, true)
        createViewModel()
