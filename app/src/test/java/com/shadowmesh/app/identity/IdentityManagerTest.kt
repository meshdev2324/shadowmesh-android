...
class IdentityManagerTest {
    ...
    fun `activate failure flow - invalid token`() = runTest {
        val response = ActivationResponse(
            ...
            vpnConfig = null, serverLocation = null
        )
    ...
    fun `registerPasskey success`() = runTest {
        coEvery { apiClient.passkeyRegisterBegin("user") } returns "challenge"
        val challenge = identityManager.registerPasskey("user")
    ...
    fun `loginWithPasskey success`() = runTest {
        ...
        val response = ActivationResponse(
            ...
            devicesRemaining = 1, vpnConfig = null, serverLocation = null
        )
