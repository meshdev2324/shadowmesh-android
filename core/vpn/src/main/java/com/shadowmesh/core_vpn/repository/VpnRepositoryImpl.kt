...
class VpnRepositoryImpl @Inject constructor(
    ...
    override suspend fun connect(node: VpnNode, config: VpnConfig, dnsLeakProtection: Boolean) {
        ensureInitialized()
        wireGuardService.connect(node, config, dnsLeakProtection = dnsLeakProtection)
    }
