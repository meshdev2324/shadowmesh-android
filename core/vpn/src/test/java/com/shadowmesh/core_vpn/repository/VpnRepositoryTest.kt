...
class VpnRepositoryTest {
    ...
    fun `initialization restores state when code exists`() = runTest(testDispatcher) {
        ...
        VpnRepositoryImpl(vpnManager, apiClient, killSwitchManager, securityEventLogger, nodeCache, wireGuardService, networkClient, secureStorage, context, testDispatcher)
    ...
    fun `connect and disconnect delegate to wireGuardService`() = runTest {
        ...
        repository.connect(node, config, true)
        coVerify { wireGuardService.connect(node, config, dnsLeakProtection = true) }
